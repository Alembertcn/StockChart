...
object DefaultIndexTextFormatter {
    val MA: (idx: Int, value: Float?) -> String = { idx, value ->
        "MA${
            com.wb.stockchart.DefaultIndexParams.MA.split(",")
                .map { it.trim() }[idx]
        }:${value?.let { NumberFormatUtil.formatPrice(it) } ?: "——"}"
    }
    val EMA: (idx: Int, value: Float?) -> String = { idx, value ->
        "EMA${
            com.wb.stockchart.DefaultIndexParams.EMA.split(",")
                .map { it.trim() }[idx]
        }:${value?.let { NumberFormatUtil.formatPrice(it) } ?: "——"}"
    }
    ...
    val RSI: (idx: Int, value: Float?) -> String = { idx, value ->
        "RSI${
            com.wb.stockchart.DefaultIndexParams.RSI.split(",").map { it.trim() }[idx]
        }:${value?.let { NumberFormatUtil.formatPrice(it) } ?: "——"}"
    }
...
object DefaultIndexStartText {
    const val MA = "MA"
    const val EMA = "EMA"
    const val BOLL = "BOLL(${com.wb.stockchart.DefaultIndexParams.BOLL})"
    const val MACD = "MACD(${com.wb.stockchart.DefaultIndexParams.MACD})"
    const val KDJ = "KDJ(${com.wb.stockchart.DefaultIndexParams.KDJ})"
}
