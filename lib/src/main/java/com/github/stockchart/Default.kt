...
object DefaultIndexTextFormatter {
    val MA: (idx: Int, value: Float?) -> String = { idx, value ->
        "MA${
            com.github.stockchart.DefaultIndexParams.MA.split(",")
                .map { it.trim() }[idx]
        }:${value?.let { NumberFormatUtil.formatPrice(it) } ?: "——"}"
    }
    val EMA: (idx: Int, value: Float?) -> String = { idx, value ->
        "EMA${
            com.github.stockchart.DefaultIndexParams.EMA.split(",")
                .map { it.trim() }[idx]
        }:${value?.let { NumberFormatUtil.formatPrice(it) } ?: "——"}"
    }
    ...
    val RSI: (idx: Int, value: Float?) -> String = { idx, value ->
        "RSI${
            com.github.stockchart.DefaultIndexParams.RSI.split(",").map { it.trim() }[idx]
        }:${value?.let { NumberFormatUtil.formatPrice(it) } ?: "——"}"
    }
...
object DefaultIndexStartText {
    const val MA = "MA"
    const val EMA = "EMA"
    const val BOLL = "BOLL(${com.github.stockchart.DefaultIndexParams.BOLL})"
    const val MACD = "MACD(${com.github.stockchart.DefaultIndexParams.MACD})"
    const val KDJ = "KDJ(${com.github.stockchart.DefaultIndexParams.KDJ})"
}
